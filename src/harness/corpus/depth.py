...
class DepthWorld:
    ...
    def _plane(self, name: str, parallax: float, coverage: float, layer: int,
        ...
        ## Почему текстура из нескольких октав, а не из блоков одного размера

        Это вторая поправка в мире, и она такого же рода, как первая. Текстура была
        блоками одного размера (6 px), и на ней **никакой** иерархический метод не
        может найти большое смещение — не «работает плохо», а не может в принципе.
        Пирамида масштабов начинает с грубого уровня, чтобы поймать смещение целиком;
        но блоки одного размера при уменьшении усредняются в ровное поле, и на грубом
        уровне ловить нечего. На тонком уровне окно меньше смещения — классическая
        апертура.

        Замер, сдвиг ровно 34 px, синтетическая текстура вне мира:
        ...
        Ждали +34. То есть дело было не в методе. У настоящей сцены структура есть на
        всех масштабах сразу — силуэт дерева, ветка, лист, — и мир без этого проверяет
        не метод, а свою собственную вырожденность.
        """
        h = int(self.height * 3)
        w = int(self.width * 3)
        ...
        # Текстура внутри области: несколько октав, от крупной к мелкой. Веса падают
        # с масштабом, как у естественных изображений: крупное несёт больше энергии.
        ...
        weight_total = 0.0
        for octave, weight in ((blob * 4, 0.45), (blob * 2, 0.25),
                               (blob, 0.18), (max(1, blob // 2), 0.12)):
            cells = rng.integers(40, 200, (h // octave + 2, w // octave + 2),
                                 dtype=np.uint8)
            big = np.repeat(np.repeat(cells, octave, axis=0), octave, axis=1)[:h, :w]
            acc += weight * big.astype(np.float64)
            weight_total += weight
        fine_big = np.clip(acc / weight_total, 40, 200)
