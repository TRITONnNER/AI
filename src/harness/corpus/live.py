...
def ingest(source: str | Path, corpus: str | Path, *, kind: str,
    ...
    has_audio = (source / "audio" / "index.jsonl").exists()
    if not has_audio:
...
def plan_text() -> str:
    ...
    rows = ["Шесть записей для живого корпуса (TASK-03, часть 2).",
            "Формат один и тот же, обрабатываются одним и тем же кодом.", ""]
    for name, k in KINDS.items():
        rows.append(f"  {name:<12} {k['title']} — {k['duration']}")
        rows.append(f"               зачем: {k['why']}")
        rows.append(f"               как:   {k['how']}")
        rows.append(f"               команда: harness record ЗАПИСЬ --actor human "
                    f"--frames N   # затем: harness ingest ЗАПИСЬ --kind {name}")
        rows.append("")
    rows += [
