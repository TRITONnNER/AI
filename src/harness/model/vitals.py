...
def from_journal(journal: Journal, *, profile: Profile,
    ...
    out.add("prediction_error_mean", None, "0..1",
            "нет источника",
            "ошибка предсказания в журнал не пишется: она живёт в PredictionError "
            "внутри процесса. Чтобы считать её по журналу, нужно прогонять кадры "
            "заново — это делает bench, а не сводка")
    ...
    out.add("mood_valence", None, "-1..1", "нет источника",
            "настроение не журналируется: оно производное от объективных величин и "
            "пересчитывается, а не хранится. В сводке его нет намеренно — иначе "
            "человек стал бы сравнивать прогоны по настроению, а не по поведению")
