...
def from_journal(journal: Journal, *, profile: Profile,
    ...
    # --- открытые вопросы (показатель 11) -----------------------------------
    # Считаются по журналу, а не по живому реестру: реестр — кэш, и если бы показатель
    # брался из него, дрейф между следом и памятью стал бы неизмерим ровно в том месте,
    # где он важнее всего — в перечне того, чего агент не понял.
    from .questions import from_journal as questions_from_journal

    q = questions_from_journal(journal).stats()
    out.add("questions_open", q["open"], "вопросов", "записи HYPOTHESIS журнала",
            "сколько загадок открыто: гипотез, для которых агент не смог построить "
            "проверку. Не очередь дел — очередь дел это `hypotheses_in_work`, и "
            "смешивать их нельзя (MIND.md, раздел 5)",
            independence=count("вопрос", q["asked"]))
    out.add("questions_resolved_share", q["resolved_share"], "доля",
            "записи HYPOTHESIS журнала" if q["asked"] else "нет источника",
            "какая доля заданных вопросов разрешилась задним числом — после того как "
            "появилось недостающее. Ноль при заданных вопросах означает «ни одна "
            "загадка ещё не разрешилась», а отсутствие числа — «вопросов не было»",
            independence=share("вопрос", q["asked"]) if q["asked"]
            else count("вопрос", 0))
    out.add("questions_mean_wait", q["mean_wait_entries"], "записей",
            "записи HYPOTHESIS журнала" if q["resolved"] else "нет источника",
            "сколько записей журнала проходит от постановки вопроса до разрешения. Это "
            "и есть скорость, с которой мир отвечает на вопросы, которых агент сам "
            "закрыть не мог",
            independence=mean("вопрос", q["resolved"]) if q["resolved"]
            else count("вопрос", 0))
    out.add("questions_coverage", q["coverage"], "доля",
            "записи HYPOTHESIS журнала" if q["asked"] else "нет источника",
            "какая доля вопросов объявила, чего им не хватает. Остальные механизмом "
            "переоткрытия не вернутся никогда, и без этого числа реестр, переоткрывший "
            "три вопроса из трёх объявленных при сорока слепых, выглядит работающим "
            "на сто процентов (инвариант 31)",
            independence=share("вопрос", q["asked"]) if q["asked"]
            else count("вопрос", 0))
    ...
    out.add("params_defective", bad, "настроек", "harness.params.audit",
            f"мёртвые, однобокие и «только отчёт» — {bad} из {total} настроек схемы. "
            "Однобокий опаснее мёртвого: он читается на одном пути и молча врёт на "
            "другом, и именно так живая запись заявила кадр в 36 раз меньше "
            "настоящего. Число в постоянном отчёте затем, чтобы оно не росло "
            "незамеченным (TASK-10, часть 3)",
            independence=count("настройка", total))

    return out


def _dead_params() -> tuple[int, int]:
    """Сколько настроек не работают и сколько их всего. Статикой, по AST.

    Считается по коду, а не по журналу, и это здесь законно по той же причине, что и
    два числа выше: утверждение не про прогон, а про устройство механизма. Способ —
    статический обход: он видит пути, которые прогон не проходил, а однобокость иначе
    установить нечем. Второй способ (счётчики) в постоянный отчёт не идёт: он зависит
    от того, что именно прогон успел пройти, и число прыгало бы от прогона к прогону,
    ничего не сообщая о механизме.
    """
    from ..params import audit, defect_count

    findings = audit()
    return defect_count(findings), len(findings)
