    ...
    def observe(self, fp: tuple[int, ...], seq: int, *, seconds_per_seq: float = 1.0,
        ...
        if self.current is not None:
            if self.current != place.id:
                # Уход: время прохода — **длительность этого шага**, а не накопленное
                # с прихода в место.
                #
                # Раньше здесь стояло `_since_seq`, то есть время от входа в
                # исходное место. Тогда ребро измеряло «сколько я простоял здесь
                # перед уходом» плюс один шаг, а не «сколько занимает этот переход»,
                # — и у петли, десятью строками ниже, про это же написано прямо, но
                # для ухода правило не соблюдалось. Найдено при сверке `mu` двух
                # маршрутов (`TASK-04`, часть 2): выяснилось, что весь разброс `mu`
                # на сиде 17 — это разное время простоя разведки, а не свойство
                # мира. То есть `sigma` ребра измеряла поведение разведчика.
                #
                # Сдвиг числа, который эта правка предъявляет (инвариант 25): `mu`
                # всех одношаговых рёбер становится ровно одинаковым, а `sigma`
                # нулевой везде, а не на пяти сидах из восьми. Это делает вакуумность
                # сверки `mu` на этом мире явной вместо мнимого разброса.
