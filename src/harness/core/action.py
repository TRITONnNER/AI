...
def too_risky(caution: float, threshold: float) -> bool:
    """Выше ли осторожность порога. Одно сравнение на весь проект.

    Сравнение **строгое**, и это не косметика. Незнание стоит ровно единицу
    (`caution == 1.0` при `n == 0`), а верх шкалы порога — тоже единица. При нестрогом
    сравнении `threshold = 1.0` оставляет порог включённым для всего непробованного,
    то есть контроль «порог снят» не снимает ничего. Замер обратимости (TASK-32 C)
    именно на это и налетел: контрольный прогон был вакуумен, и совпадение счётчиков
    объяснялось отчасти им, а не только тем, что порог никто не читает.

    Функция одна, потому что читателей у порога несколько — планировщик, лепет, отчёт
    карты тела, — и разошедшиеся сравнения означали бы, что «порог снят» на одном пути
    и не снят на другом.
    """
    return caution > threshold
