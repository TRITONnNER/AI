...
class LoopbackAudio:
    ...
    def start(self) -> None:
        ...
        # Частота, с которой устройство **на самом деле** отдаёт отсчёты. Драйвер вправе
        # подставить свою: попросили 48 кГц, получили 44.1, и `read()` об этом не скажет
        # ни словом. Дальше пеленг считается по разнице каналов в отсчётах, длительность
        # блока — делением на заявленную частоту, и всё уезжает на 8.8 % молча.
        # Расхождение — отказ, а не предупреждение (TASK-10, часть 4): поле профиля,
        # описывающее свойство записи, обязано сходиться с записью. Проверка стоит
        # **вне** `try` намеренно: вход открылся, и путать это с «не открылся» нельзя.
        actual = self.actual_rate
        if actual is not None and actual != int(self.rate):
            self.stop()
            raise BackendUnavailable(
                f"устройство отдаёт {actual} Гц, а профиль заявляет {self.rate} Гц. "
                "Поставьте audio_rate по устройству или выберите другое устройство: "
                "считать длительность и пеленг по неверной частоте значит ошибаться "
                "во всём звуке ровно в отношение частот")

    @property
    def actual_rate(self) -> int | None:
        """Настоящая частота открытого потока, либо `None`, если её не спросить."""
        if self._stream is None:
            return None
        try:
            return int(round(float(self._stream.samplerate)))
        except Exception:
            return None
