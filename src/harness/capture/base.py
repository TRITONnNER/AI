...
class Gone:
    """Источник исчез. **Это наблюдение, а не сбой захвата.**

    TASK-21, часть 6. Окно закрыли, монитор отключили, приложение свернули — для
    механизма захвата это разные ветки API, для агента одно происшествие: то, на что он
    смотрел, пропало. Различать обязательно, и различать в обе стороны:

    - исчезновение, записанное разрывом, учит агента разрыву, которого не было, и портит
      счёт целостности записи: разрыв означает «механизм не справился»;
    - сбой, записанный исчезновением, прячет поломку под видом события мира. Искать её
      будут в поведении агента, а она в захвате.

    Поэтому у чтения источника **четыре** разных ответа, а не два:

    | Ответ | Что случилось | Куда идёт в журнал |
    |---|---|---|
    | `Frame` | кадр | запись кадра |
    | `UNCHANGED` | кадр тот же | отметка со временем |
    | `GONE` | источник пропал | наблюдение (`Kind.DEVICE`) |
    | `None` | поток кончился | конец записи |
    | исключение | механизм сломался | разрыв (`Kind.CAPTURE_GAP`) |

    Ложность (`__bool__`) та же, что у `UNCHANGED`: `if frame:` без явной проверки
    обрабатывает исчезновение как «кадра нет», и это безопасная сторона ошибки.
    """
...
#: Единственный экземпляр. Сравнивать через `is`.
GONE = Gone()


class CaptureBroken(BackendUnavailable):
    """Механизм захвата сломался на ходу. **Разрыв**, а не событие мира.

    Отдельный тип нужен, чтобы отличать «сломалось во время записи» от «не запустилось
    вовсе»: первое оставляет частичный корпус, годный для разбора, второе — ничего.
    Наследуется от `BackendUnavailable`, потому что обработчики, ловящие отказ механизма,
    обязаны ловить и этот случай тоже.
    """
