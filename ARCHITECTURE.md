...
снят подписью, а не выбрасыванием. `settings.SYNTHETIC_ONLY` объявляет такие настройки
списком (не по префиксу `world_`: `randomize_world` префикса не имеет, а
`world_layer_tolerance` читает разделитель слоёв, работающий и на живом), а
`Profile.diff` помечает каждую строку полем `live`. `Profile.live_diff` оставляет только
те различия, которые могли повлиять на живую запись.
