...
PARTS: tuple[Part, ...] = (
    ...
    Part("Профиль и два хеша", ("core/profile.py", "core/settings.py"),
         ...
         "129 настроек: единицы, границы, форк журнала"),
    ...
    Part("Библиотека навыков", ("behaviour/skills.py",), ("skill",),
         "макросы добыты из журнала; обратимость цепочки — свой вопрос"),
    ...
    Part("Планировщик", ("behaviour/planner.py",), ("plan", "travel", "rehears"),
         "на подтверждённой модели доходят все планы: 40 из 40"),
    ...
    Part("Слои взгляда: тау и частоты", ("vision/layers.py",),
         ...
         "дальность по параллаксу не работает: четыре измеренные попытки, "
         "верным находится только средний план"),
...
MISSING: tuple[tuple[str, str], ...] = (
    ...
    ("Оставшиеся 6 % планов",
     "планы от трёх шагов доходят 224 из 239 на восьми сидах. Провалы — там, где "
     "расхождение предсказаний нашлось, но разделить его не удалось ни по яркости, "
     "ни по одной ячейке: делить нечем, и выдумывать порог нельзя"),
    ("Второе деление одного места",
     "место делится один раз: `splits` держит один порог на место. Состояние мира, "
     "различимое двумя признаками сразу, разделится только по первому"),
    ...
    ("Дальность (ближний/средний/дальний план) по параллаксу",
     "четыре измеренные попытки: верным находится только план, совпавший с найденным "
     "сдвигом. Нужен настоящий оптический поток — пирамида масштабов, субпиксельная "
     "точность, регуляризация с разрывами. Таблица замеров в harness.vision.layers"),
