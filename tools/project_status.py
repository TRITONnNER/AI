...
PARTS: tuple[Part, ...] = (
    ...
    Part("Узнавание места: три средства", ("model/recognition.py",),
         ("recognition", "background", "novelty", "sequence", "adaptive",
          "repainted", "split", "invariant_part"),
         "24 прогона (8 сочетаний × 3 сида): ни одно из трёх средств не уменьшило "
         "граф — новизна 1.00×, фон 1.03×, отрезок 1.62×. Единственный выигрыш — "
         "ложные слияния 0.40 % → 0.09 %. Разделение отпечатка убрало задвоение "
         "карточек: 48 → 12 при истине 12",
         "живой источник не размечает части отпечатка: `Split` умеет их различать, "
         "но подавать ему нечего, кроме объявленной замером нагрузки"),
    Part("Критерий запуска A4", ("launch.py",),
         ("launch", "vacuous", "seven", "unchecked"),
         "семь пунктов считаются по журналу и замерам, у пункта четыре исхода: "
         "вакуумно выполненный не засчитывается. Сейчас 0 из 7",
         "пункты 1, 2, 4, 6, 7 не проверены: нет живой записи и нет прогона с "
         "консолидацией и целями"),
