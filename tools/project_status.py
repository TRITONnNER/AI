...
PARTS: tuple[Part, ...] = (
    ...
    Part("Метрика конфабуляции", ("model/confabulation.py",),
         ...
         "на корпусе из 6 объяснённых действий даёт 67 %; без реплик даёт None, а "
         "не ноль"),
    ...
    Part("Профиль и два хеша", ("core/profile.py", "core/settings.py"),
         ...
         "140 настроек: единицы, границы, форк журнала"),
...
MISSING: tuple[tuple[str, str], ...] = (
    ...
    ("Вытеснения, закладок и насыщения по sigma",
     "сделана только структура уровней, которая их допускает: понижение по "
     "лестнице, потолок, резерв нулевого уровня. Ранжирование и выбор, что "
     "понижать, — следующая задача (TASK-02, «чего в этой задаче нет»)"),
    ("Замера конфабуляции на настоящем агенте",
     "метрика считается и даёт число, но реплики планировщика в прогонах пока "
     "ставятся вручную: контура, который объясняется, ещё нет. Число 67 % — с "
     "искусственного корпуса, а не с поведения"),
